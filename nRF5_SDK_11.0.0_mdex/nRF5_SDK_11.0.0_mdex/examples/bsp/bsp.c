...
#define ADVERTISING_LED_OFF_INTERVAL			1500
...
#define ADVERTISING_WHITELIST_LED_OFF_INTERVAL	1500

#define ADVERTISING_SLOW_LED_ON_INTERVAL		200
#define ADVERTISING_SLOW_LED_OFF_INTERVAL		4000

#define CONNECTED_LED_ON_INTERVAL				200
#define CONNECTED_LED_OFF_INTERVAL				4500
...
static uint32_t bsp_led_indication(bsp_indication_t indicate)
{
    ...
    switch (indicate)
    {
        ...
        case BSP_INDICATE_ADVERTISING:
			//APP_LOG("[step0] led(0) mask=0x%08x, OUT:0x%08x, IS_ON:0x%08x, OUTSET:0x%08x, OUTCLR:0x%08x \r\n", 
			//		BSP_LED_0_MASK, NRF_GPIO->OUT, LED_IS_ON(BSP_LED_0_MASK),
			//		(BSP_LED_0_MASK) & (LEDS_MASK &  LEDS_INV_MASK),
			//		(BSP_LED_0_MASK) & (LEDS_MASK & ~LEDS_INV_MASK));


			LEDS_OFF(LEDS_MASK & ~BSP_LED_0_MASK & ~m_alert_mask);
/*
			APP_LOG("[step1] led(0) mask=0x%08x, OUT:0x%08x, IS_ON:0x%08x, OUTSET:0x%08x, OUTCLR:0x%08x \r\n", 
					BSP_LED_0_MASK, NRF_GPIO->OUT, LED_IS_ON(BSP_LED_0_MASK),
					(BSP_LED_0_MASK) & (LEDS_MASK &  LEDS_INV_MASK),
					(BSP_LED_0_MASK) & (LEDS_MASK & ~LEDS_INV_MASK));
*/
			...
			//printf("(a1)lm->0x%08x, ~lim->0x%08x, out->0x%08x, out^~lim->0x%08x, lm&(out^~lim)->0x%08x \r\n", 
			//		LEDS_MASK, (~LEDS_INV_MASK), (NRF_GPIO->OUT),
			//		(NRF_GPIO->OUT ^ (~LEDS_INV_MASK)), (LEDS_MASK & (NRF_GPIO->OUT ^ (~LEDS_INV_MASK))));

			//APP_LOG("[step2] led(0) mask=0x%08x, OUT:0x%08x, IS_ON:0x%08x, OUTSET:0x%08x, OUTCLR:0x%08x \r\n\r\n", 
			//		BSP_LED_0_MASK, NRF_GPIO->OUT, LED_IS_ON(BSP_LED_0_MASK),
			//		(BSP_LED_0_MASK) & (LEDS_MASK &  LEDS_INV_MASK),
			//		(BSP_LED_0_MASK) & (LEDS_MASK & ~LEDS_INV_MASK));
        ...
        case BSP_INDICATE_CONNECTED: // CONNECTED_LED_ON_INTERVAL
            LEDS_OFF(LEDS_MASK & ~BSP_LED_0_MASK & ~m_alert_mask);
            // LEDS_ON(BSP_LED_0_MASK);
            m_stable_state = indicate;
            break;
